...
class FocusShutdownUserBot(sleekxmpp.ClientXMPP):
    def __init__(self, jid, password, focus):
        ...
        self.add_event_handler("session_start", self.shutdown_focus, threaded=True)
        ...
        try:
            ...
            if resp["type"] == "result":
                ...
                while True:
                    ...
                    logging.info(
                        "There are " + str(conf_count) + " conferences in " "progress"
                    )
                    if conf_count == "0":
                        break
                    else:
...
if __name__ == "__main__":
    ...
    optp.add_option("-s", "--server", dest="server", help="JID to use")
    optp.add_option("-j", "--jid", dest="jid", help="JID to use")
    optp.add_option("-p", "--password", dest="password", help="password to use")
    optp.add_option("-f", "--focus", dest="focus", help="JID of the focus component")
